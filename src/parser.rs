    ...
    fn new(path: &std::path::Path) -> Page {
        ...
        let date = if let Some(captures) = date_re.captures(&name) {
            ...
            Some(chrono::NaiveDate::from_ymd_opt(
                ...
                get_number(3, "day").try_into().unwrap()).unwrap())
        } else {
            None
        };
...
pub fn load_pages(top_path: &std::path::Path) -> Vec<Page> {
    ...
    // Create backlinks.
    let backlinks : Vec<(Name, Title, Vec<Name>)> =
        pages_map
        .iter()
        .map(|(name, page)|
            (name.clone(),
    ...
    fn create_test_pages(dir: &std::path::Path) {
        ...
        std::fs::write(
            ...
            b"<h1>Need a title element else it breaks</h1><br>
                ...
                <p>Let's create a another self cycle <a href=\"2022-02-22-test_page_3.html\">another link</a></p>")
